...
from Functions.utils import loss_func_CH_GD
...
def features_calc(X, u, u_mu):
    ...
    features = torch.cat([u_t, u_xx, mu_xx], dim=1)
...
class params_equation(torch.nn.Module):
    ...
    def calc_mu(self, features, u_pred):
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (local_batch, output) in loop:
        ...
        ## equation residual
        mu_cal = model_params.calc_mu(features, u_pred)
        residual = model_params(features)

        ## loss evaluation
        loss, loss_data, loss_eq, loss_mu = loss_func_CH_GD(
            output, u_pred, residual, u_mu, mu_cal
        )
    ...
    if epoch % 1 == 0:
        print(
            "It: %d, Loss: %.3e, Loss_data: %.3e, Loss_eq: %.3e, Loss_mu: %.3e, Lambda_1: %.3e, Lambda_2: %.3e"
            % (
                epoch,
                loss_epoch,
                loss_data_epoch,
...
plt.plot(params_collect2[:, 0], label=("$\lambda_1$"))
...
plt.plot(params_collect2[:, 1], label=("$\lambda_2$"))
